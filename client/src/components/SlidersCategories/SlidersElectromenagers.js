...
import categoryElectromenager from '../CATEGORIES/categoryNivel/categoryElectromenager';
...
const SliderElectromenager = () => {
  ...
  useEffect(() => {
    if (subsubcategorySlug && subsubcategorySlug !== 'undefined') {
      ...
      const parentCategory = categoryElectromenager.categories.find(
        cat => categoryElectromenager.subcategories[cat.id]?.some(
      ...
      const category = categoryElectromenager.categories.find(
  ...
  const getCurrentData = () => {
    if (currentLevel === 1) {
      return categoryElectromenager.categories || [];
    } else if (currentLevel === 2 && selectedCategory) {
      return categoryElectromenager.subcategories[selectedCategory.id] || [];
    }
    return [];
  };

  // Calcular distribución en dos filas - EXACTO como SliderVehicule
  ...
  const getIconPath = (iconName) => {
    if (currentLevel === 1) {
      return `/icons/electromenager/${iconName}`;
    } else {
      return `/icons/electromenager/${selectedCategory?.id || 'default'}/${iconName}`;
    }
  };
  ...
  const handleItemClick = (item) => {
    if (currentLevel === 1) {
      if (item.hasSublevel) {
        ...
        history.push(`/electromenager/${item.id}/1`);
      }
    } else if (currentLevel === 2) {
      // En nivel 2: navegar a página de sub-subcategoría
      history.push(`/electromenager/${selectedCategory.id}/${item.id}/1`);
    }
  };
  ...
  const handleBackClick = () => {
    if (currentLevel === 2) {
      ...
      history.push('/electromenager/1');
    }
  };

  // Renderizar fila de iconos - IDÉNTICO a SliderVehicule
      ...
      >
        {row.map((item) => {
          ...
          return (
            ...
            >
              {/* CONTENEDOR DE ICONO - EXACTO como SliderVehicule */}
              <div
                style={{
              ...
              {/* Nombre - EXACTO como SliderVehicule */}
              <div style={{
                textAlign: 'center',
                width: '100%',
                padding: '0 2px'
  ...
  return (
    <div ref={containerRef} className="category-grid-container">
      {/* Card contenedor - EXACTO como SliderVehicule */}
      <div style={{
        position: 'relative',
        maxWidth: '1400px',
        margin: '0 auto',
        ...
        <div style={{
          ...
          textAlign: 'center',
          display: 'none' // Ocultado como en SliderVehicule
        }}>
          <div style={{
            display: 'inline-flex',
            alignItems: 'center',
      ...
      {/* Estilos CSS COPIADOS EXACTAMENTE de SliderVehicule */}
      <style>{`
